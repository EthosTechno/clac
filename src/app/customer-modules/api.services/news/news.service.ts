...
export class NewsService implements OnDestroy {
  ...
  getNewsDetail(requestModel: any, endPoint: string): Observable<ResponseModel> {
    if (environment.isSimpleJson) {
  ...
  getNewsAdd(requestModel: any, endPoint: string): Observable<ResponseModel> {
    if (environment.isSimpleJson) {
      return this.baseService.get('assets/jsonData/' + endPoint + '.json')
        .pipe(map<HttpResponse<any>, any>(response => {
          return response.body;
  ...
  getNewsDelete(requestModel: any, endPoint: string): Observable<ResponseModel> {
    if (environment.isSimpleJson) {
      return this.baseService.get('assets/jsonData/' + endPoint + '.json')
        .pipe(map<HttpResponse<any>, any>(response => {
          return response.body;
